import collections
"""
modul pour des données de type conteneurs spécialisés, qui apportent 
des alternativess aux conteneurs natifs de python plus généraux 
dict, list, set, tuple.
"""
...
def _tokenize(content): # trouver à quoi sert cette fonction
    return re.findall(r'([A-Za-z0-9\.\-_]+[:;\{\}])', contents)

"""
Enlever les lignes de commentaire. 
But les titre de chaque états commence par des ### ... 
donc comment les différenciers par la suite ?? idk 
"""
...
def _parse_list(tokens):
    parsed_list = [] # liste des éléments à lire 
    while True : 
        token = tokens.popleft() # lecture des éléments de la gauche vers la droite (pq c'est important ?)
        if token == ';':
...
def _parse_map(tokens): # qu'est ce que la "map" pour le model d'un hmm ? 
    parsed_map = collections.OrderedDict() # sous classe de dico qui garder en mémoir l'ordre dans lequel les entrées on été ajoutés
    while True:
        token = tokens.popleft() 
        # 2e fois que je vois le popleft ... pourquoi "left"
...
"""
En regardant sur internet, il n'y aurait pas de différence entre utiliser 
pop ou popleft. La complexité est de O(1). LEs deux méthodes sont optimales.
Cependant, le developpeur à important le modul "collection" pour une raison 
La même chose est vrai pour "append". 
DONC on va garder cela pour le moment. 
"""

"""
Nomenclature d'un état 
########## State ##############
begin { stat_name
                input : proba
                output : proba
...
"""
def _parse_state(tokens): # lecture des états
    state_name = tokens.popleft()
    tokens.popleft() # "{" 
     

    parsed_state = {} # enregistrer au fur et a mesure les états lus. 
    while True:
        token = tokens.popleft() 
        if token == '}': 
            return tokens, (state_name, parsed_state)
        if token in ('trans', 'only'):
            tokens, value = _parse_map(tokens) # pourquoi ?  
            if value is None:
                tokens, value = _parse_list(tokens) 
        elif token in ('type', 'end'):
            value = int(tokens.popleft()) # valeur == probabilité de l'état ? 
        else:
            value = tokens.popleft()
...
def _parse_header(tokens):
    ...
    while True:
        token = tokens.popleft()
        if token == '}':
            break # quitte la boucle lorsque l'on sort de l'état 
        header[token] = tokens.popleft() # associe 
        tokens.popleft() # ";"
...
def parse(file_like):
    ...
    return header, _to_matrix_form(header['alphabet'],_normalize_states(states))